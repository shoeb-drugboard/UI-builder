...
import { Product } from '../State/cartStore';
...
const Cards = (
    ...
    const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useInfiniteQuery({
        ...
        queryFn: async ({ pageParam = 1 }) => {
            const res = await axios.get(`${PATH}/ecommerce/products?limit=10&page=${pageParam}`);
            return res.data;
        },
    ...
    return (
        <AnimatePresence mode="wait">
            <motion.div
                ...
                initial="hidden"
                animate="show"
                className={cn('h-full w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-4 gap-6', className)}
            >
                {data?.pages.map((page) => (
